...
const ImageContainer = ({ title, setImage, image }) => (
  <>
    {title}
    <fieldset className="shadow-lg shadow-gray-400 rounded-lg border-primary p-5 border w-full">
      <div className="border-dashed grid grid-cols-8 rounded-md bg-gray-100 border-[#DCAC00] py-3 px-3 border-2">
...
const RawLabel = ({ id, title }) => (
  <label htmlFor={id}>
    {title}
    <fieldset className="shadow-lg shadow-gray-400 cursor-pointer rounded-lg border-primary p-5 border w-full">
      <div className="border-dashed flex items-center justify-center flex-col rounded-md bg-gray-100 border-[#DCAC00] py-5 border-2">
        <div className="mb-1">
          <UploadIcon />
        </div>
        <div className="text-sm mb-0 font-bold text-[#9C7A00]">Drag & Drop</div>
        <div className="mt-2 text-xs text-gray-400 mb-2">
